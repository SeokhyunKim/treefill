...
dtinr-roots-own [num-trgs num-rcvd t-gen-detect fills num-detected-trgs cur-round]
dtinr-nodes-own [num-trgs num-rcvd t-gen-detect fills notified]
dtlf-nodes-own  [num-trgs num-rcvd t-gen-detect num-detect notified]
...
to-report is-fills-full
  let i 0
  while [ i < tree-arity ]
  [
    if 0 = (array:item fills i) [ report 0 ]
    set i i + 1
  ]
  report 1
end
...
to-report level
  report floor(log ((tree-arity - 1) * who + 1) tree-arity)
end
to-report rand-upper-level-node
  let lvl [level] of self - 1
  let first-id (tree-arity ^ lvl - 1) / (tree-arity - 1)
  let last-id-plus-one (tree-arity ^ (lvl + 1) - 1) / (tree-arity - 1)
  let num-nds-lvl last-id-plus-one - first-id
  let id-offset random num-nds-lvl
  report turtle (first-id + id-offset)
end
to-report child-idx [i]
  report who * tree-arity + i + 1
end
to-report get-child [i]
  report turtle (who * tree-arity + i + 1)
end
to-report parent-idx
  report (ceiling(who / tree-arity) - 1)
end
to-report get-parent
  report turtle (ceiling(who / tree-arity) - 1)
end
...
to setup
  clear-all
  set num-nodes tree-arity ^ L
  set DetectTree-height L - 1
  ...
  set-default-shape dtinr-roots "circle"
...
to gen-node-tree
  ...
  [
    set num-nodes-gen-height (tree-arity ^ gen-height)
    set xord 1
  ...
  let tid 0
  let num-dnodes (tree-arity ^ DetectTree-height - 1) / (tree-arity - 1)
  let child 0
  ...
  [
    ...
    while [ i < tree-arity ]
    [
      ask turtle tid [ create-link-with [get-child i] of self ]
      set i i + 1
    ]
    set tid tid + 1
  ]
...
to gen-other-nodes
  set num-oth-nodes num-nodes - length nodes-list
  let num-created 0
  ...
  let i 1
  let j 1
  while [ i <= num-row and (num-created < num-oth-nodes) ]
  [
    set j 1
    while [ j <= num-col and (num-created < num-oth-nodes) ]
    [
      create-nodes 1 [ 
        set color green
        if num-nodes < 50 [ set size 10 ]
        if 50 <= num-nodes and num-nodes < 100 [ set size 5 ]
      ]
      let new-node turtle (count turtles - 1)
...
to init-root-of-DetectTree ;; the procedure for the root node and the inner nodes of DetectTree
  set color green
  set fills array:from-list n-values tree-arity [0]
  set cur-round 1
...
to prepare-round [round-num]  
  ...
  ;; if average arity of the spanning tree is same with the tree-arity, the num of ticks for msg propagation through the spanning tree is log(N, arity)
  ;let i 1
  ;let s 1
  ;let id 0
  ;while [ id < num-nodes ]
 ...
 ; ]
  ...
  let que2 []
  let lvl 0
  set que1 lput root que1
  ask root [reset-node new-t]
    
  while [(length que1) > 0] [
    foreach que1 [
      ask ? [reset-node new-t]
      if lvl < DetectTree-height [
        let k 0
        while [ k < tree-arity ] [
          set que2 lput ([get-child k] of ?) que2
          inc-num-msgs 1
          set k k + 1
        ]          
      ]
    ...
    set que1 que2
    set que2 []
    if lvl < DetectTree-height [ tick ]      
  ]
    
  let nn length nodes-list
  let i nn - num-oth-nodes
  while [ i < nn ] [
    ask turtle i [set-new-t new-t]
    inc-num-msgs 1
    set i i + 1
  ]
  tick  
...
to go
  ...
  let notify-new-round [is-fills-full] of root
  ...
  set notify-new-round [is-fills-full] of root
  if notify-new-round = 1 [
    let new-round ([rnd] of root + 1)    
    if new-round > 1 [
      ask root [
        set num-detected-trgs (num-detected-trgs + (length dtlf-nodes-list) * tree-arity * t-gen-detect)
      ]
    ]    
    ifelse ([total-detected-trgs] of root >= (num-triggers + num-added-trgs)) [
      ...
      file-write (word ([rnd] of root) " " num-msgs " " max-rcvd)
      file-close
      stop
    ][
...
to proc-detect ;; dtlf-node procedure
  ...
  [    
    ...
    if num-detect >= tree-arity [
      set notified 1
      ;print (word "notify full: " who)
      inc-num-msgs 1
      ask [get-parent] of self [ inrproc-full-notify [who] of myself ]
    ]
  ]  
end
...
to inrproc-detect [from] ;; dtinr-node procedure  
  ...
  ifelse (not (who = 0)) and (not (from = [parent-idx] of self))
  [
    let target 0
    let i 0
    while [ i < tree-arity ]
    [
      if 0 = (array:item fills i) [
        set target [child-idx i] of self
        set i tree-arity - 1
      ]
      set i i + 1
    ]
...
to forward-rand-node-of-upper-level  
  ...
  ;print (word "forward rand upper-lvl nodes from " who " to " rupnd)
